...
ENV PHP_7_2_VERSION 7.2.0

RUN CONFIGURE_OPTS="--enable-phar --with-libdir=/lib/x86_64-linux-gnu --with-gmp --enable-intl --with-pear" PHP_BUILD_INSTALL_EXTENSION="apcu=5.1.8 imagick=3.4.3 mongodb=1.3.4" phpenv install $PHP_7_0_VERSION && rm -r /tmp/php-build
RUN CONFIGURE_OPTS="--enable-phar --with-libdir=/lib/x86_64-linux-gnu --with-gmp --enable-intl --with-pear" PHP_BUILD_INSTALL_EXTENSION="apcu=5.1.8 imagick=3.4.3 mongodb=1.3.4" phpenv install $PHP_7_1_VERSION && rm -r /tmp/php-build
RUN CONFIGURE_OPTS="--enable-phar --with-libdir=/lib/x86_64-linux-gnu --with-gmp --enable-intl --with-pear" PHP_BUILD_INSTALL_EXTENSION="apcu=5.1.8 imagick=3.4.3 mongodb=1.3.4" phpenv install $PHP_7_2_VERSION && rm -r /tmp/php-build
